...
(deftest parsing-negative-numbers-larger-than-minus-1
  (let [expected (pp/parse "task foo() { return -0.5; }")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest empty-program
  (let [expected (pp/parse "")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest blink13
  (let [expected (pp/parse "task default() running 1/s {\n\ttoggle(D13);\n}")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest procedure-with-argument
  (let [expected (pp/parse "proc blink(arg0) {\n\tturnOn(arg0);\n\tdelayS(1);\n\tturnOff(arg0);\n}")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest function-with-arguments
  (let [expected (pp/parse "func default(arg0, arg1) {\n\treturn (arg0 % arg1);\n}")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest functions-with-calls-and-globals
  (let [expected (pp/parse "var global;\n\nfunc forIncrease(from, to, by) {\n\tfor i = from to to by by {\n\t\tglobal = (global + 1);\n\t}\n\treturn global;\n}\n\nfunc run() {\n\tvar temp = forIncrease(1, 10, 0.5);\n}")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))

(deftest operator-precedence
  (let [expected (pp/parse "func operate(arg0, arg1) {\n\treturn arg0 + arg1**2*3;\n}")
        actual (pp/parse (cg/print expected))]
    (is (= expected actual))))
